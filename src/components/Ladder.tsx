...
export function Ladder({ courseId, initialBlocks, onBlocksChange }: LadderProps) {
  ...
  const setBlockLoading = (id: number, loading: boolean) => {
    setLoadingIds((prev) => (loading ? [...prev, id] : prev.filter((i) => i !== id)))
  }
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      try {
        ...
        toast({
          title: "Couldn't save order. Try again.",
          variant: 'destructive',
        })
      } finally {
  ...
  const handleAddBlock = async (kind: 'CONTENT' | 'ASSESSMENT') => {
    const title = kind === 'CONTENT' ? 'New Content Block' : 'New Assessment Block'
    setIsAdding(true)
    try {
      ...
      >
        <SheetContent side="right">
          {selectedBlockId !== null && (
            <ContentEditor blockId={selectedBlockId} onBlocksChange={onBlocksChange} />
          )}
        </SheetContent>
      </Sheet>
    </>
